...
def compute_flag_pos(num_objects):
    ...
    return on_pos, connected_pos, curr_pos

def compute_num_nodes(num_objects):
    on_pos, connected_pos, curr_pos = compute_flag_pos(num_objects)
    return (1 << curr_pos)
...
#actions: build flags
#move x,y
#Pre = connected(x,y) wedge on(x) wedge x != y
#Add = on(y)
#Del = on(x)
...
def compute_init_nodes(num_objects, pattern):
    ...
    for j in range(0, num_objects):
        initnodes.append((graph << connected_pos) | (1 << (on_pos + j)))
...
with open('navgraph1ops2_noconstraints.dfa', 'w') as f:
    f.write(output_string)

with open('navgraph1ops2_noconstraints.dot', 'w') as f:
...
for num_objects in range (2,9):
    for pattern in [73,26,57,48]:
        ...
        for i,node in enumerate(output_nodes):
            ...
            output_string += str(connections) + node_string + "\n"
        output_string = "dfa " + str(num_nodes) + " -1\n" + "1 move\n" + "1 0\n" + output_string
        filename = "navgraph1ops" + str(num_objects) + "pat" + str(pattern) + ".dfa"
