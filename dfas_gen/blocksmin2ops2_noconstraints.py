...
def compute_flag_pos(num_objects):
    ...
    return handfree_pos, grabbed_pos, clear_pos, on_pos, curr_pos

def compute_num_nodes(num_objects):
    handfree_pos, grabbed_pos, clear_pos, on_pos, curr_pos = compute_flag_pos(num_objects)
    return (1 << curr_pos)

#all_flags = num_nodes - 1
#encoding bittable: handfree,grabbed0,grabbed1,clear0,clear1,on00,on01,on10,on11

#actions: build flags
#unstack x,y
#Pre = onblock(x,y) wedge clear(x) wedge handfree()
#Add = grabbed(x) wedge clear(y)
#Del = onblock(x,y) wedge handfree()
...
#Add = onblock(x,y) wedge handfree()
...
#put x
#Pre = grabbed(x)
#Add = onblock(x,x) wedge handfree()
#Del = grabbed(x)
...
def compute_init_nodes(num_objects):
    handfree_pos, grabbed_pos, clear_pos, on_pos, curr_pos = compute_flag_pos(num_objects)
    initnode = 0
    initnode = initnode | (1 << (handfree_pos))
    for j in range(0, num_objects):
    ...
    while todo_nodes:
        ...
        for flags in put_action_flags:
                if current_node & flags[0] == flags[0]:
                    ...
                    if not ((target in output_nodes) or (target in todo_nodes)):
                        todo_nodes.append(target)
                    current_edges.append([" put ",target])
