...
def effu_3(a, p, args, val):
    return clingo.Function('eff_used', [clingo.Number(a), clingo.Function('', [clingo.Number(p),clingo.Function('',map_to_number(args))]), clingo.Number(val)])
...
class STRIPSSchema:
    ...
    def add_fluent_true_val(self, pred, args):
    ...
    def add_static_true_val(self, pred, args):
        self.add_predicate(pred)
    ...
    def create_from_clingo(cls, symbols : List[clingo.Symbol]):
        ...
        for symbol in symbols:
            ...
            elif symbol.match(*VAL_1):
                k, = symbol.arguments
                p, objs = k.arguments
                schema.add_static_true_val(p.number, map_to_int(objs.arguments))
            elif symbol.match(*VAL_2):
                k, s = symbol.arguments
                p, objs = k.arguments
                vals.append(((p.number, map_to_int(objs.arguments)), s.number))
        ...
        for (p, oo), s in vals:
            if s == root:
                schema.add_fluent_true_val(p, oo)
    ...
    def get_schema(self):
        ...
        for action, arity in self._actions.items():
            ...
            for p, arg, val in self._effs[action]:
                ...
                symbols.append(effu_3(action, p, arg, val))
